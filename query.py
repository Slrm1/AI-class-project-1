...
Answer ONLY from the retrieved context below. If the context does not support the answer, say: 'I don't have enough information in the Howard CS knowledge base to answer that.'
Do not use outside knowledge to fill gaps. Keep answers concise and cite the source file names used in your answer.
...
def answer(question: str):
    ...
    for i, (doc, meta, distance) in enumerate(results, 1):
        context_parts.append(
            f"[Chunk {i} | {meta['source_file']} | distance={distance:.4f}]\n{doc}"
        )
    context = "\n\n".join(context_parts)
...
if __name__ == "__main__":
    ...
    for _, meta, distance in results:
        print(f"- {meta['source_file']} (distance {distance:.4f})")
